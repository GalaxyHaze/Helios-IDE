...
public:
        ...
        : QMainWindow(parent), m_initialContextSetup(true)
    {
        ...
        // Set initial context root and open default file
        ...
        // Initial context state saved
        m_initialContextSetup = false;
        saveContextState();

        // Tab change updates breadcrumbs, status, and find bar
        ...
        connect(m_tabWidget, &QTabWidget::tabCloseRequested, this, [this](int idx) {
            ...
            if (ed->document()->isModified()) {
                ...
                if (result == QMessageBox::Save) {
                    bool was = (ed == currentEditor()); // current tab is this one
        ...
        // Go to definition connected per-tab in connectEditorSignals()
    }
...
private slots:
    ...
    void newProject()
    {
        ...
        // Ask what to do with current workspace
        QDialog dlg(this);
