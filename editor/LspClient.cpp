...
void LspClient::changeDocument(const QString &uri,
  ...
  for (const LspTextChange &change : changes) {
    ...
    start["character"] = change.range.start.character;

    QJsonObject end;
    end["line"] = change.range.end.line;
    end["character"] = change.range.end.character;
  ...
  sendRequest("textDocument/hover", params, [this](const QJsonObject &resp) {
    ...
    if (!range.isEmpty()) {
      ...
      info.range.start.character =
          range["start"].toObject()["character"].toInt();
...
void LspClient::requestSignatureHelp(const QString &uri,
...
void LspClient::requestFormatting(const QString &uri) {
  ...
  sendRequest(
      "textDocument/formatting", params, [this](const QJsonObject &resp) {
...
int LspClient::nextId() { return ++m_nextId; }
...
void LspClient::handleResponse(const QJsonObject &msg) {
  ...
  if (msg.contains("error") && !msg["error"].isNull()) {
    ...
    qWarning() << "LSP error:" << err["code"].toInt()
